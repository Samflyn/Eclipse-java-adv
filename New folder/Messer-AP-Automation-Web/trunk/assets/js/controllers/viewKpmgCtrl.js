 ...
 */
app.controller('viewKpmgInvoiceCtrl', ["$scope", "$rootScope", "$http","$filter", "$stateParams", "ViewInvoiceService","$timeout","$cookies","SendToErpService", function ($scope,$rootScope, $http,$filter, $stateParams,ViewInvoiceService,$timeout,$cookies,SendToErpService ) {
	$scope.loginDetails={};
    ...
    $scope.showLog = function(){
    	if($scope.data.headerId){ 
	 	$http({
	        method: 'GET',
	        url: $rootScope.ctx +'/showAuditLog/'+ $scope.data.headerId,
	        data: $scope.data,
