...
app.controller('generateCustomerInvoiceCtrl', ["$scope","$rootScope", "$http","$cookies","$filter","GenerateInvoiceService", "$timeout", "SendToErpService", "refreshToErpService","$state","$stateParams","$sce","$window",function ($scope,$rootScope, $http,$cookies,$filter,GenerateInvoiceService, $timeout, SendToErpService, refreshToErpService,$state,$stateParams,$sce,$window) {
	...
	$scope.loginDetails.userId=$rootScope.get('userId');
	        	            	...
	        	            	$scope.data.pdffile= $scope.pdffile1;
	        	            	 ...
	        	            	 if($scope.data.items.length==0){
		        		           $("#saveMsg").show().html('<div class="alert alert-danger"<strong>Invoice contains at least one item</strong> </div>');
	        	            		 ...
	        	            		 $("#saveMsg").show().html('');
	        	            		 if($scope.data.purchaseOrderNo && $scope.data.bpName){
	        	            			 $scope.data.errorDesc="";
	     		    		    	}
	        	            		 console.log($scope.data)
		        	            	GenerateInvoiceService.generateInvoice($scope.data,$scope.loginDetails).then(function(response){
		        		               $("#saveMsg").show().html('<div class="alert alert-success"<strong>Invoice Saved Sucessfully</strong> </div>');
	 ...
	 $scope.uploadImage=function(input) {
 		if (input.files && input.files[0]) {
 			...
 			/*reader.onload = function (e) {
 				$scope.data['pdffile'] =e.target.result.split(",")[1] 
 			}*/
 			 ...
 			 reader.onload = function (e) {
 	        	...
 	        	// $('[ng-controller="generateCustomerInvoiceCtrl"]').scope().pdffile=e.target.result.split(",")[1] ;
	...
	$scope.bpObj={};         
	        	 ...
	        	 formData.append('invoiceType', "PI");
	     		 $http({
	     		        ...
	     		        headers: {'Content-Type': undefined,'registeredId':$rootScope.registeredId,'loginDetails':JSON.stringify($scope.loginDetails)}
		    		    	...
		    		    	if(!$scope.data.purchaseOrderNo || !$scope.data.bpName){
		    		    		$scope.data.errorDesc="Vendor Name OR/AND Purchase order number not found"
		    		    		$("#saveMsg").show().html('<div class="alert alert-danger"<strong>'+$scope.data.errorDesc+'</strong> </div>');	
		    		    	}
		    		   
	    					/*
		     		 		
		    		    	$scope.bpObj = $scope.businesspatnersList[0]; */
		    		    	/*if($scope.bpObj) {
	        ...
	        $scope.sendToErp = function(){	  
	        	...
	        	console.log("in generate customer invoice control");
	        SendToErpService.sendToErpObj($scope.data,$scope.loginDetails).then(function (response) {
	        	if(response.data.Status.toUpperCase()=="MATCHED"){
        ...
        if($stateParams.headerId){
        	...
        	GenerateInvoiceService.editInvoice($scope.loginDetails).then(function (response) {
        	      $scope.data = response.data;
