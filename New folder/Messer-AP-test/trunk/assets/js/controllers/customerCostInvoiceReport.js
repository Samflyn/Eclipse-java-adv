 ...
 */
app.controller('customerCostInvoiceReportCtrl', ["$q","$compile","costInvoicReportsService","DTOptionsBuilder", "DTColumnBuilder","$scope","$rootScope", "$http","$cookies","$filter", "$timeout", "$stateParams","$window",function ($q,$compile,costInvoicReportsService,DTOptionsBuilder, DTColumnBuilder,$scope,$rootScope, $http,$cookies,$filter, $timeout,$stateParams) {
	$scope.data ={};
	var curr = new Date(); 
				...
				if(!($scope.data.fromDate && $scope.data.toDate) && !($scope.data.returnPeriod) && !($scope.data.invNo) ){
					...
					if($scope.data.fromDate && $scope.data.toDate){
					    ...
					    if(difference>0){
					    	...
					    	costInvoicReportsService.exportInvoceReport($scope.data.fromDate,$scope.data.toDate,$scope.loginDetails,$scope.data.returnPeriod,$scope.data.invNo,$scope.data.workStatus).then(function (result){
	...
	vm.dtOptions = DTOptionsBuilder.fromFnPromise(function () {  
		...
		costInvoicReportsService.getInvoceReport($scope.data.fromDate,$scope.data.toDate,$scope.loginDetails,$scope.data.returnPeriod,$scope.data.invNo,$scope.data.workStatus).then(function (result) {  
	...
	function actionsHtml(data, type, full, meta) {
		...
		return '<button type="button"  class="btn btn-primary margin-right-5" ng-click="viewInvoice(customerCostReports.singleRecord['+full.mailConfigId+'])">' +'<i class="fa fa-eye"></i>' +'</button>';
	}
	$scope.report={};
	...
	function reloadData(fromDate,toDate,returnPeriod,invNo,workflow)
	{
		$scope.dtInstance.reloadData(function() {
			var defer = $q.defer();  
			costInvoicReportsService.getInvoceReport(fromDate,toDate,$scope.loginDetails,returnPeriod,invNo,workflow).then(function (result) {
				defer.resolve(result.data);  
			});  
			return defer.promise; 
		}, true); 
